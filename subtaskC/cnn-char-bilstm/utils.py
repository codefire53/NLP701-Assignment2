...
def res_plot(data, desc="", p=3):
    ...
    for idx, t in enumerate(["loss", "f1"]):
        ...
        ax.legend()
    metric_name = f'../images/plot[{datetime.now().strftime("%y%m%d%H%M")}]-Ep{epochs}{desc}.png' # noqa: E501
...
def cls_report(best_model, val_loader):
    """Classification report + Confusion Matrix."""
    y_test, y_pred = [] , []

    with torch.no_grad():
        for sentence, label, sent_lens in val_loader:
    ...
    sns.heatmap(cm, annot=labels, fmt="s", cmap="Blues", cbar=False,
                xticklabels=[0, 1], yticklabels=[0, 1], linewidths=0.1 )
